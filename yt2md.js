...
// @require      https://code.jquery.com/jquery-4.0.0.js
// @grant        GM_download
...
let transcriptPath = "L:\\Obsidian\\YoutubeVids\\";
...
(function () {
    ...
    if (true) {
        ...
        #yto-y2mmarkdown-btn.no-transcript { background-color: #d9a106 !important; }
        #yto-y2mmarkdown-btn.no-transcript:hover { background-color: #f0b429 !important; }
        ...
        #yto-btn-settings { background: #555; color: #fff; font-size: 12px; padding: 6px 12px; }
    ...
    // --- Server Manager --- // Note that if functions are called repeatedly very quickly they may fail because of throttling
    class ServerManager {
        constructor() {
        ...
        async checkVideoExists(id) {
            if (!this.config.enabled)
                return false;
            consolelog(`[Y2M] checkVideoExists: ${this.getBaseUrl()}/EverythingExists/?`+id);
            return new Promise((resolve) => {
                GM_xmlhttpRequest({
                    method: 'GET',
                    url: `${this.getBaseUrl()}/EverythingExists/?`+id,
                    timeout: 5000,
                    onload: (response) => {
                        consolelog(`[Y2M] CheckVideoExists: `+ ((response.status === 200)?"Found!":"Not Found!"), response);
                        resolve((response.status === 200), response);
                    },
                    onerror: () => {
                        consolelog(`[Y2M] CheckVideoExists: ERROR!!!`);
                        resolve(false, "False");
                    }
                });
            });
    ...
    class TranscriptExtractor {
                    ...
                    if (text.includes('captionTracks')) {
                        ...
                        if (match) {
                            ...
                            if (tracks && tracks.length > 0) {
                                ...
                                consolelog('[Y2M] TranscriptExtractor: ', englishTrack);
        ...
        async extractTranscriptFromDOM() {
            return new Promise((resolve) => {
                ...
                if (!transcriptButton) {
                    consolelog("[Y2M] TranscriptExtractor: No Button!")
                    resolve(null);
                    return;
                }
                ...
                setTimeout(() => {
                    ...
                    if (segments.length === 0) {
                        consolelog("[Y2M] TranscriptExtractor: No Segments!")
                        var m = $("ytd-engagement-panel-section-list-renderer[target-id=PAmodern_transcript_view] > div#content div#contents div#contents transcript-segment-view-model");
                        if (m.length == 0) {
                            consolelog("[Y2M] TranscriptExtractor: Trying alternative transcript view!");

                            m = $("yt-section-list-renderer[data-target-id=PAmodern_transcript_view] div#contents transcript-segment-view-model");
                            //<yt-section-list-renderer class="ytSectionListRendererHost style-scope ytd-engagement-panel-section-list-renderer ytSectionListRendererHostEnableScroll ytSectionListRendererStickyHeader" data-target-id="PAmodern_transcript_view" data-resize-observer-id="0" panel-content-visible="" panel-target-id="PAmodern_transcript_view">
                            if (m.length == 0) {
                              consolelog("[Y2M] TranscriptExtractor: Null transcript view!");
                              resolve(null);
                              return;
                            }
                        }
                        // Transcript may have alternate structure

                        var transcript2 = ""
                        $(m).each((k,e) => {
                            var timestamp = $("div.ytwTranscriptSegmentViewModelTimestamp", e).text();
                            var text = $("span", e).text();
                            transcript2 = transcript2 + `[${timestamp}] ${text}\n`;
                        })

                        resolve(transcript2.trim() || null);
                        return;
                    }
        ...
        async getTranscript() {
            ...
            consolelog('[Y2M] TranscriptExtractor: ', transcript);
            return transcript;
        }
    }
    ...
    class UIManager {
        ...
        async updateServerStatus() {
            ...
            if (online) {
                ...
                var id = window.location.href.replace('https://www.youtube.com/watch?v=',"").substring(0,11);
                consolelog("[Y2M] updateServerStatus: Video ID = " + id);
                if (id.length == 11)
                {
                  var found = await this.serverManager.checkVideoExists(id)
                  if (found)
                  {
                    statusEl.textContent = 'Server Online (✅)';
                  } else
                  {
                      statusEl.textContent = 'Server Online (⚠️)';
                  }
                }
        ...
        async checkFileHasTranscript(filename) {
            try {
                const content = await this.serverManager.readFile(filename);
                consolelog('[Y2M] Error reading file checking transcript!', filename);
                if (!content) return false;
                // Check if file contains Transcript section
                return content.includes('## Transcript');
            } catch (e) {
                consolelog('[Y2M] Error checking transcript:', e);
                return false;
            }
        }
        ...
        async createButton(container) {
            ...
            if (!config.enabled) {
                ...
                filename = this.generateFilename({
                    ...
                    consolelog("[Y2M] createButton filename " + filename)

                    if (await this.serverManager.checkConnection() && await this.serverManager.fileExists(filename)) {
                        fileExists = true
                    }
            }
            ...
            if (fileExists) {
                ...
                const hasTranscript = await this.checkFileHasTranscript(filename);
                consolelog('[Y2M] File has transcript:', hasTranscript);
                if (!hasTranscript) {
                  btn.classList.add('no-transcript');
                } else {
                }
            } else {
        ...
        async openModal(existingFilename = null, category = 'Uncategorized') {
            ...
            const pathCPButton = document.createElement('button');
            pathCPButton.id = 'yto-btn-pathcp';
            pathCPButton.textContent = "Copy FN"
            pathCPButton.onclick = async () => {
              const loc = await this.serverManager.getFilePath(existingFilename);
              GM_setClipboard(transcriptPath + loc);
            }
            rowDiv.appendChild(pathCPButton);

            const TSButton = document.createElement('button');
            TSButton.id = 'yto-btn-transcriptbutton';
            TSButton.textContent = "Copy TS"
            if (!this.existingData || !this.existingData.transcript) TSButton.style.color = "#EE2222"
            rowDiv.appendChild(TSButton);
            ...
            if (serverEnabled) {
                ...
                if (fileExists && online) {
                    ...
                    if (existingContent) {
                        ...
                        existingTranscript = this.parseTranscript(existingContent);
                        //consolelog("[Y2M] Existing transcript length:", existingTranscript ? existingTranscript.length : 0);

                        if (existingTranscript) TSButton.style.color = "#33EE44"
            ...
            // Enable TS copying of transcript
            TSButton.onclick = async () => {
              var ts = existingTranscript;
              if (!ts) {
                ts = await this.transcriptExtractor.getTranscript();
                TSButton.style.color = "#33EE44"
              }
              GM_setClipboard(ts);
            };
            ...
            try {
                ...
                if (includeTranscript) {
                    ...
                    if (this.existingData && this.existingData.transcript) {
                        ...
                        const btn = document.getElementById('yto-y2mmarkdown-btn');
                        if (btn) btn.classList.remove('no-transcript');
                    } else {
